...
def fetch_paper_by_arxiv_id(arxiv_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"[方式1] 通过 arXiv ID 获取论文: {arxiv_id}")
        ...
        if not paper:
            print(f"[方式1] 未找到 arXiv ID: {arxiv_id}")
            return None
        ...
        # 获取 BibTeX（优先 DBLP，失败后用 arXiv）
        # DBLP 会尝试使用 title + authors 获取更准确的 BibTeX
        bibtex = get_bibtex_enhanced(
            title=paper.title, authors=authors_str, arxiv_id=arxiv_id
        )
        ...
        print(f"[方式1] ✅ 成功获取论文: {result['title'][:50]}...")
        return result

    except Exception as exc:
        print(f"[方式1] ❌ 获取论文失败: {exc}")
        import traceback
...
def search_arxiv_by_title_and_author(
    ...
    """
    使用标题和作者搜索 arXiv 论文（参考 test4.py）
    ...
    try:
        ...
        print(f"[方式2.3] 使用标题+作者搜索 arXiv: [{query}]")
        ...
        print(f"[方式2.3] ✅ 找到匹配论文: {result['title'][:50]}...")
        return result

    except Exception as exc:
        print(f"[方式2.3] ❌ 搜索失败: {exc}")
        import traceback
...
def search_arxiv_by_title_only(title: str) -> Optional[Dict[str, Any]]:
    ...
    """
    try:
        print(f"[方式2.4] 使用标题搜索 arXiv: {title[:50]}...")
        ...
        print(f"[方式2.4] ✅ 找到匹配论文: {result['title'][:50]}...")
        return result

    except Exception as exc:
        print(f"[方式2.4] ❌ 搜索失败: {exc}")
        import traceback
...
def process_uploaded_pdf(pdf_path: str, filename: str) -> Optional[Dict[str, Any]]:
    ...
    if arxiv_id_from_filename:
        print(f"[方式2.1] 从文件名提取到 arXiv ID: {arxiv_id_from_filename}")
        result = fetch_paper_by_arxiv_id(arxiv_id_from_filename)
        if result:
            print(f"[方式2.1] ✅ 成功通过 arXiv ID 获取信息")
            return result
        print(f"[方式2.1] ❌ 通过 arXiv ID 获取失败，继续降级策略")
    ...
    if pdf_metadata["arxiv_id"]:
        print(f"[方式2.2] 从 PDF metadata 提取到 arXiv ID: {pdf_metadata['arxiv_id']}")
        result = fetch_paper_by_arxiv_id(pdf_metadata["arxiv_id"])
        if result:
            print(f"[方式2.2] ✅ 成功通过 arXiv ID 获取信息")
            return result
        print(f"[方式2.2] ❌ 通过 arXiv ID 获取失败，继续降级策略")

    # ========================================================================
    # 2.3 使用 PDF metadata 的 '/Title' 和 '/Author' 搜索
    # ========================================================================
    if pdf_metadata["title"] and pdf_metadata["authors"]:
        print(f"[方式2.3] 使用 PDF metadata 的标题和作者搜索")
        ...
        if result:
            print(f"[方式2.3] ✅ 成功找到匹配论文")
            return result
        print(f"[方式2.3] ❌ 未找到匹配，继续降级策略")

    # ========================================================================
    # 2.4 使用 PDF 解析提取 title，然后搜索
    # ========================================================================
    print(f"[方式2.4] 使用 PDF 解析提取标题")
    ...
    if not pdf_metadata["title"]:
        ...
        if title:
            print(f"[方式2.4] 通过字体大小提取到标题: {title[:50]}...")
            ...
            with open(pdf_path, "rb") as file:
                ...
                if len(pdf_reader.pages) > 0:
                    ...
                    if full_text:
                        ...
                        if title:
                            print(f"[方式2.4] 通过文本分析提取到标题: {title[:50]}...")
        except Exception as exc:
            print(f"[方式2.4] 文本提取失败: {exc}")
    ...
    if title:
        ...
        if result:
            print(f"[方式2.4] ✅ 成功找到匹配论文")
            return result
        print(f"[方式2.4] ❌ 未找到匹配论文")
    else:
        print(f"[方式2.4] ❌ 无法提取标题")

    print(f"[方式2] ❌ 所有策略都失败，无法获取论文信息")
    return None
